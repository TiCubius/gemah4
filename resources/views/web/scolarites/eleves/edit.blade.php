...
@php($title = "Édition de {$eleve->nom} {$eleve->prenom}")
        ...
        <div class="col-12">
            <form class="mb-3" action="{{ route("web.scolarites.eleves.update",[$eleve]) }}" method="POST">
                ...
                <div class="form-group custom-control custom-checkbox">
                    ...
                    @else
                        <input name="joker" id="joker" class="custom-control-input" type="checkbox">
                    @endif
    ...
    @foreach($eleve->responsables as $responsable)
        @if($responsable->eleves->count() <= 1 )
            ...
            <div class="custom-control custom-checkbox">
                <input id="delete-responsables[]" class="custom-control-input" name="delete-responsables[]"
                       value="{{$responsable->id}}" type="checkbox">
                <label class="custom-control-label"
                       for="delete-responsables[]">Supprimer {{ "{$responsable->nom} {$responsable->prenom}"}}</label>
            </div>
        @endif
    @endforeach
@endcomponent
