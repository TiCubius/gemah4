	...
	@foreach($eleve->responsables as $responsable)
		<body>
			<header>
				...
				<section id="informations" style="display: inline-block; width: 125px; font-size: 10px; text-align: right; margin-top: -80px;">
					<p>
						CONVENTIONS <br>
			...
			<section id="content" style="font-size: 13px;">
				...
				<section id="article-1" style="text-align: justify;">
				...
				<section id="article2" style="text-align: justify;">
					...
					<p>
						Pour <b>l'année scolaire {{ Carbon\Carbon::parse("last year")->format("Y") }} / {{ Carbon\Carbon::now()->format("Y") }}</b> est mis à disposition le matériel suivant appartenant à l’Etat :
					</p>
				...
				<section id="article3" style="text-align: justify;">
					...
					<p>Il ne sera demandé aucune contribution financière au bénéficiaire à l'exclusion des consommables indispensablesau fonctionnement du matériel.</p>
				</section>

				<section id="article4" style="text-align: justify;">
					<h2>Article 4 : conditions de mise à disposition</h2>

					<p>Le bénéficiaire s'engage à utiliser le bien qui lui est confié uniquement dans le cadre du travail scolaire dans l'établissement ou à son domicile (article 1880 du Code civil). Il s’engage à porter à la connaissance du service gestionnaire (direction des services départementaux de l’éducation nationale de le Loire, service informatique) tout sinistre affectant le matériel prêté. Le service étudiera alors les modalités de prise en charge des réparations nécessaires. Toutefois s’il s’avère que le matériel a subit des dommages importants et ne peut être réparé, il ne sera pas remplacé plus d’une fois. Toute modification dans la composition de la liste du matériel fera l'objet d'un avenant à la présente convention (retrait ou adjonction).</p>
				</section>
				...
				<section id="article5" style="text-align: justify;">
					...
					<p>
						...
						@if($decision->date_limite && $decision->date_notif)
							Le prêt de matériel prendra fin le {{ $decision->date_limite->format("d/m/Y") }} en vertu de la décision du {{ $decision->date_notif->format("d/m/Y") }}.
						@endif
					</p>
				</section>
				...
				<section>
					<p>Dans le cas d'un matériel restant en permanence sur le lieu de scolarisation : signature du Chef d'établissement ou du représentant de la commune (articles 1921, 1927 et suivants du Code civil).</p>
				</section>
				...
				<section class="text-center">
					<p><b>Cette convention, après signature de la famille et du secrétaire général, doit être conservée par la famille</b></p>
				</section>
			</section>
		</body>
	@endforeach
